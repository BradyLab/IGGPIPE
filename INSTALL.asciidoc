...
to log out and back in for a PATH change to take effect). I verified that Jellyfish
was installed and able to be run with this command:

  jellyfish --version

The parameter CMD_JELLYFISH in the allPathParameters.oursfile is already set to
'jellyfish', which is the command needed to run the Jellyfish program. You
shouldn't need to change it.

Also, you may need to change the parameter JELLYFISH_HASH_SIZE, also located in the
allPathParameters.ours file. The value already in the file will usually work fine.
However, if you
